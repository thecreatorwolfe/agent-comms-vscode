...
- Codex sessions are woken with `codex queue --thread <id> --message <text>`, which hands the ping to the session itself rather than typing into its terminal, so the normal Codex TUI reacts to inbound Slack without switching to an app-server host UI. A busy session consumes the ping at its next turn boundary. Terminal wake-up injection remains the fallback when the thread cannot be resolved or the delivery cannot be confirmed in the transcript. Turn it off with `agentComms.codexQueueDelivery` if you want injection only, and set `agentComms.codexCliPath` when `codex` is not on the extension host's PATH.
