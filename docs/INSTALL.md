...
- Codex sessions are woken with `codex queue --thread <id> --message <text>`, which hands the ping to the session itself rather than typing into its terminal, so the normal Codex TUI reacts to inbound Slack without switching to an app-server host UI. A busy session consumes the ping at its next turn boundary. Set `agentComms.codexCliPath` when `codex` is not on the extension host's PATH.
- Terminal prompt injection is retired. It dropped messages whenever the terminal was not in a state to accept synthetic input. `agentComms.codexTerminalInjectionFallback` turns it back on if a queued delivery cannot be confirmed; it is off by default.
- Spawned Codex agents receive their persona through `mcp_servers.agent-comms.env` on the launch command, because Codex sanitizes the environment it gives MCP servers. Without that, the bridge connects unnamed and the hub cannot route to the spawn-assigned persona until the agent renames itself.
