...
- A freshly spawned Codex agent that answers "persona registration is still required" is not receiving its persona. Read the bridge process environment with `ps eww -p <codex-server.js pid>` and look for `AGENT_COMMS_PERSONA`. If it is missing, the launch command is not carrying the `-c mcp_servers.agent-comms.env.AGENT_COMMS_PERSONA=...` override, because Codex sanitizes the environment it hands to MCP servers.
- Repeated `prompt=inject` means the thread could not be resolved or confirmed, and the injection fallback is switched on. Check that `codex` is on the extension host's PATH (or set `agentComms.codexCliPath`), that the agent's working directory matches the session's `cwd`, and that the session started after the agent registered. The hub matches sessions by working directory and registration time.
